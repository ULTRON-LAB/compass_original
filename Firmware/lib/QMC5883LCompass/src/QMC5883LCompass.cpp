...
/*	重要！！！！！！！！！！！！！！！！！！！！！
此庫已由BXCHEN改為支援QMC5883P，如需更動請自行參閱使用晶片之database
各項參數可在初始化程式碼自行更改，復位、寄存器、模式、OSR等等寫入值也請自行釐清
*/
...
void QMC5883LCompass::init(){
   ...
   Wire.begin();
    ...
    // 1. 執行軟復位（寄存器0x0B的bit7）
    _writeReg(0x0B, 0x80); // 軟復位命令
    delay(50); // 延遲等待復位完成
    
    // 2. 配置控制寄存器2（0x0B）
    //    bit[7:6]: 保留
    //    bit[5]: 自檢使能 (0=禁用)
    //    bit[4:3]: 量程選擇 (00=2G, 01=8G, 10=12G, 11=30G)
    //    bit[2:1]: 設置／復位模式 (00=開啟)
    //    bit[0]: 保留
    _writeReg(0x0B, 0x08); // 8G量程 (01000 -> 0x08)
    ...
    //    bit[3:2]: ODR (輸出數據率)
    //    bit[1:0]: 模式控制
	//連續模式 @10Hz:0x03;@50Hz:0x07;@100Hz:0x0B;@200HZ:0x0F
	//4倍OSR抗雜訊:0x13;0x17;0x1B;0x1F;
//	Bit:   7    6    5    4    3    2    1    0
//         |    |    |    |    |    |    |    |
//         +----+----+----+----+----+----+----+
//         OSR2     OSR1     ODR      MODE
    ...
    _writeReg(0x0A,0x03); // 0b00001111 (連續模式+200Hz)
    
    // 4. 配置符號寄存器（0x29）
    _writeReg(0x29, 0x06); // 定義XYZ軸符號
...
void QMC5883LCompass::read() {
    Wire.beginTransmission(_ADDR);
    Wire.write(0x00);  // 从状态寄存器开始
    int err = Wire.endTransmission();
    if (!err) {
        Wire.requestFrom(_ADDR, (byte)7);  // 读取7字节 (状态 + XYZ)
        
        // 读取状态寄存器
        byte status = Wire.read();
        
        // 读取数据 (LSB first, 按X,Y,Z顺序)
